...
集合型数据的特点是元素无序且不重复。常用于成员测试、去重及数学运算中（交集、并集、补集）。`Redis` 的 key 可以视为 `string` 型的集合。
...
>
> 集合中也有一个命令可以枚举其中的所有元素——`SMEMBERS` 命令。但与 `HGETALL` 类似，当集合存在海量元素时可能卡死服务器，此时推荐使用 `SSCAN` 命令。
...
有序集合型较之前介绍的其他类型更复杂。所谓有序，是指该集合中的元素都有一个可用于排序的权重值，可以按顺序读取元素值。这样的原生排序特性在需要总是用到排序的场景中显得十分方便。
...
(integer) 6
127.0.0.1:6379> ZREVRANGE totalranking 0 -1 WITHSCROES
...
注意，这里的 `ZUNIONSTORE` 是对两个有序集作并集运算（`ranking:restaurants` 与 `ranking2:restaurants`），其中前者权重为 1，后者为 2，表示各有序集中的元素权重分别乘以 1、乘以 2 后，相同元素的权重再求和（默认的 `AGGREGATE` 聚合方式），最后将得到输出结果存入新的有序集 `totalranking`，并展示带权重值的运算结果。`ZUNIONSTORE` 命令的语法如下：

```bash
ZUNIONSTORE destination numkeys key [key ...] [WEIGHTS weight [weight ...]] [AGGREGATE SUM|MIN|MAX]
```

此外，`ZREVRANGE` 是按权重作逆序排列；顺序排列要用 `ZRANGE key min max` 命令。
