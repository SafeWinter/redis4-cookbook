...
随着智能手机的日渐普及，定位相关的服务也越来越流行。地理位置 API 在 `Redis` 3.2 版后引入，用于在有关定位的场景中存储及查询这些地理空间坐标。演示如下：
...
据 `Redis` 的官方文档，出于性能考虑，`GEORADIUS` 的时间复杂度为 ***O(N+log(M))***，其中 `N` 是由圆心位置与半径划定的圆形区域内的元素个数。因此，要想获得出色的性能，应将一个查询中的半径参数设置得尽可能小，以覆盖较少的点。

更多 `GEOHASH` 的介绍详见 `Redis` 文档：https://en.wikipedia.org/wiki/Geohash，`Geo` 型的完整命令文档详见：https://redis.io/commands#geo



## 2.8 键（`keys`）的管理

总地来讲，`Redis` 中的数据是由键值对（`key-value pairs`）构成的。因此对 key 的管理也是程序开发及管理的又一基础知识。本节将演示 `key` 的管理。

演示如下：

由于需要一定规模的测试数据，需要安装 `fake2db` 及 `Python Redis` 驱动：
...
要查看 `Redis` 中有哪些 `key`，`Redis` 提供了两个 API：`KEYS` 和 `SCAN`

```bash
# KEYS pattern: show keys matched
127.0.0.1:6379> KEYS *
...
...
# SCAN cursor: incrementally retrieve values (10 records by default)
...
(integer) 0
# TYPE: return the data type of a key
127.0.0.1:6379> TYPE "company:3859"
hash
# RENAME key newkey: to rename a key into newkey
...
- 如果 `Redis` 中有相当多的键，调用 `KEYS` 命令会使 `Redis` 服务器阻塞一段时间，直到所有键都返回（虽然实操时 50000 个键的读取比原书中的 9.30 秒快很多，但对比其他“秒出”结果的命令还是慢了很多）。由于是单线程操作，`Redis` 在执行某个命令期间，已经收到的所有其他命令都必须等待该命令处理完毕。因此，从性能上讲，在生产环境使用 `KEYS` 命令是一个危险动作。推荐使用 `SCAN` 命令，如之前演示的 `HSCAN` 或 `SSCAN` 命令，都可以在不阻塞服务器的情况下迭代 `Redis` 中的键；
- `DEL` 是另一个值得注意的命令。如果要删除的键是字符串以外的数据类型，当键中的元素数量很大时，可能会遇到服务器延迟（`server latency`）。该问题的解决方案是使用 `UNLINK` 命令，区别在于 `UNLINK` 将在另一个线程而不是主事件循环线程中执行删除，因此不会阻塞事件处理；
- `RENAME` 乍一看似乎是不影响性能的；但如果要更名的新 `key` 已经存在，`RENAME` 将先删除这个 `key`。如前所述，`DEL` 可能会导致高延迟。因此，重命名的最佳做法是在判定新的 `key` 存在时使用 `UNLINK` 删除，然后再执行 `RENAME` 命令。
