...
`list` 型数据在应用开发中很常见，用于存放一组对象，也可以用作栈（`stack`）或队列（`queue`）。`Redis` 的 `key` 对应的值可以是一个字符串序列（`list of strings`）。`Redis` 中的 `list` 型数据更像是数据结构中的双向链表（`doubly linked list`）。本节介绍该类型的基本命令及数据操作。
...
(integer) 2
# get all names (LRANGE key start stop)
...
- `LPUSH`：将元素添加到列表的左端
- `RPUSH`：将元素追加到列表的右端
- `LINSERT`：在列表中的关键元素（`pivotal element`）之前或之后插入元素

以上三个方法均返回新列表的长度。无需对 `key` 赋一个初始的空列表来添加元素，当对一个不存在的 `key` 新增列表元素时，`Redis` 会自动创建一个空列表并关联到该 `key`；同样地，也无需手动删除一个空列表的 `key`，`Redis` 会自动进行回收。
...
`LPOP` 和 `RPOP` 还有对应的阻塞版（`blocking versions`）——`BLPOP` 和 `BRPOP`，并且都和对应的非阻塞版命令一样，能分别从列表的最左端或最右端弹出一个元素。语法为：
...
这个功能特性在执行调度作业的场景中十分有用：各作业方（即 `Redis` 客户端）只需在表征 `Redis` 作业列表的 `key` 上分别执行 `BLPOP` 或 `BRPOP` 命令，就能在指定的时间间隔内“监听”到作业调度方（另一个 `Redis` 客户端）的调度操作；一旦监听到作业列表加入了新的作业任务（列表元素），作业方就会按监听的先后顺序获取到对应的作业。
...
(integer) 1
dispatcher> 
# because worker-1 run BRPOP before worker-2, worker-1 get unlocked first
# while worker-2 still waiting
worker-1> brpop job_queue 0
1) "job_queue"
2) "job1"
(97.67s)
...
列表对应的 `key` 编码为 `quicklist`，可通过命令 `OBJECT ENCODING` 查看：
