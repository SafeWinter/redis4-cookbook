...
有时我们不想按元素的值排序元素，而是按在某些外部键中定义的权重排序。 例如，我们可能需要按照在 `restaurnat_rating_200` 等键中定义的评级对用户最喜欢的餐馆进行排序，其中 200 是餐馆 ID。 这也可以在 `SORT` 中完成

如果不想直接按元素的值（如 `ID`）来排序，而是按一组包含 ID 的键对应的权重值来排序，则可以使用 `SORT` 命令的 `BY pattern` 选项实现：
